...
describe('expanded TIKR metric coverage regressions', () => {
  ...
  it('adds reported line-items section with IS/BS/CF rows when present', () => {
    setLanguage('en');
    const dates = ['2024', '2025'];
    const data = {
      company: 'Line Items Corp',
      sections: {
        'Income Statement': {
          dates,
          rows: [
            { label: 'EBT Excl. Unusual Items', values: ['120', '135'], dates },
            { label: 'Income Tax Expense', values: ['-25', '-28'], dates }
          ]
        },
        'Balance Sheet': {
          dates,
          rows: [{ label: 'Total Current Assets', values: ['500', '550'], dates }]
        },
        'Cash Flow': {
          dates,
          rows: [{ label: 'Cash Interest Paid', values: ['3', '4'], dates }]
        },
        Ratios: { dates, rows: [] }
      }
    };
    ...
    const reported = results.sections.find((s) => s.id === 'reported-lines');
    expect(reported).toBeTruthy();
    expect(
      reported?.items.some((item) => item.name === 'EBT Excl. Unusual Items')
    ).toBe(true);
    expect(
      reported?.items.some((item) => item.name === 'Total Current Assets')
    ).toBe(true);
    expect(
      reported?.items.some((item) => item.name === 'Cash Interest Paid')
    ).toBe(true);
  });

  it('captures street and consensus forward series in reported line-items section', () => {
    setLanguage('en');
    const dates = ['2024', '2025'];
    const data = {
      company: 'Street Corp',
      sections: {
        ...
        'Analyst Price Targets': {
          ...
          rows: [
            {
              label: 'Media del precio objetivo de las acciones',
              values: ['280', '290'],
              dates
            }
          ]
        ...
        'Consensus Estimates': {
          dates,
          rows: [{ label: 'Ingresos', values: ['430', '450'], dates }]
        }
      }
    };
    ...
    expect(reported).toBeTruthy();
    expect(
      reported?.items.some((item) => item.name === 'Street Target Mean')
    ).toBe(true);
    expect(
      reported?.items.some(
        (item) => item.name === 'Consensus Revenue (Fwd series)'
      )
    ).toBe(true);
  });
});
