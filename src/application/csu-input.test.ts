...
describe('csu large pasted input', () => {
  it('parses and analyzes; renders dashboard when legacy renderer is available', async () => {
    ...
    const input = fs.readFileSync(
      path.resolve(process.cwd(), 'test-data/constellation-software.md'),
      'utf8'
    );
    ...
    const results =
      runAnalysisCompat.length >= 3
        ...
        : runAnalysisCompat(parsed, { industryCode: '451020' });
    ...
    if ('renderDashboard' in scoringModule) {
      ...
      const industrySelection =
        GICS_INDUSTRIES.find((item) => item.code === '451020') ??
        GICS_INDUSTRIES[0];
